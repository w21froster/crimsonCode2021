...
class PortData {
    bool verifyData(string portNumber) {
        try {
            ...
            nExc Dump: " << exc.what() << "\n\nLine: " << __LINE__ << " - " << __FUNCTION__ << endl << endl;
            return false;
        }
    }
    ...
    bool verifyData(Port& port) {
        if (port.getNumber() == (unsigned int)-1) {
            throw PortVerifcationException(port, "Invalid number detected! Value must not be -1!", __LINE__, __FUNCTION__);
        } else if (port.getServiceName() == "") {
            throw PortVerifcationException(port, "Invalid service name detected! Value must not be ''!", __LINE__, __FUNCTION__);
        }
        return true;
    }
    ...
    void importData(const string fileName) {
        ...
        if (!file.is_open()) {
            throw PortCreationException("Could not open file!", __LINE__, __FUNCTION__);
        }
